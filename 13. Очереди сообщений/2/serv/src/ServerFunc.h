 ...
 * \param mqd_t mqd_new_cleints - очередь сообщений для принятия запросов на принятие в чат
 * \param mqd_t mqd_messaging - очередь для принятия сообщений от клиентов
 * \param mqd_t mqd_exit - очередь для принятия запросов от клиентов, которые желают покинуть чат
 */
struct Server{
 ...
 * \param mqd_t mqd_client - очередь клиента, по которой будет отправляться сообщение
 * \param char* answer - сообщение
 */
void SendMsgToClient(mqd_t mqd_client, char* answer);

/*!
 * \brief Функция, прослушивающая новых клиентов. Используется во время создания соответствующего потока.
 * \param void* args - указатель на структуру сервера
 * \detail Функция ждёт, пока новый пользователь не сообщит своё имя серверу.
 * В слечае, если пользователь может быть принят на сервер пользователю отправляется ответ о том, что он принят
 * и все данные о пользователях и сообщениях. Так же в таком случае сервер оповещает всех пользователей о новом учатснике.
 * В ином случае пользователю отправляется ответ, что он был отклонён.
 */
void* NewClientListener(void* args);

/*!
 * \brief Функция, прослушиващая сообщения. Используется во время создания соотвествующего потока.
 * \param void* args - указатель на структуру сервера
 * \detail Функция ждёт, пока кто-то из пользователей не пришлёт сообщение. Затем сервер рассылает данное сообщение
 * всем пользователям, включая того пользователя, который отправил сообщение.
 */
