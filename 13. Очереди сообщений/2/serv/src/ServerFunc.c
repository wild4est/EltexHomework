...
struct Server* InitServer() {
	...
	InitQueue(&(serv->mqd_new_clients), QUEUE_NEW_CLIENT, MAX_COUNT_MSGS,
		  SIZE_BUFF_NAME, O_RDONLY);
	InitQueue(&(serv->mqd_messaging), QUEUE_MESSAGING, MAX_COUNT_MSGS,
		  SIZE_BUFF_MSG, O_RDONLY);
	InitQueue(&(serv->mqd_exit), QUEUE_EXIT, MAX_COUNT_MSGS, SIZE_BUFF_NAME,
		  O_RDONLY);
...
void SendDataToClient(mqd_t mqd_client, struct ListClient* list_client,
...
void* NewClientListener(void* args) {
	...
	while (1) {
		...
		SendDataToClient(mqd_client, serv->list_clients,
				 serv->list_msgs);
...
void* MessagingListener(void* args) {
	...
	while (1) {
		...
		ssize_t recv_r =
		    mq_receive(serv->mqd_messaging, msg, sizeof(msg), NULL);
		if (recv_r == -1) {
			printf(
			    "[X] MessagingListener: Не удалось принять "
			    "сообщение\n");
			continue;
		}
...
void* ExitListener(void* args) {
	...
	while (1) {
		char name[SIZE_BUFF_NAME];
		ssize_t recv_r =
		    mq_receive(serv->mqd_exit, name, sizeof(name), NULL);
		if (recv_r == -1) {
			printf(
			    "[X] ExitListener: Не удалось принять сообщение\n");
			continue;
		}
