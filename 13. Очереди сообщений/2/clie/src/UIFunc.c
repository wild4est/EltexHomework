...
void InitWorkspace() {
	...
	workspace.win_clients =
	    newwin(WIN_CLIENTS_H, WIN_CLIENTS_W, START_Y, WIN_MSGS_W + 2);
	box(workspace.win_clients, 0, 0);

	workspace.win_text = newwin(WIN_TEXT_H, (WIN_MSGS_W + WIN_CLIENTS_W),
				    WIN_MSGS_H + 2, START_X);

	RefreshAllWin();

	printw("Для выхода нажмите CTR + C");
	mvprintw(WIN_CLIENTS_H + 1, START_X,
		 "_____________________________________________________________"
		 "____________________");
	mvprintw(WIN_CLIENTS_H + WIN_TEXT_H + 3, START_X,
		 "_____________________________________________________________"
		 "____________________");
...
void* MessagingListener(void* args) {
	...
	while (1) {
		...
		int recv =
		    SendMsgToServer(client->mqd_messaging, client->name, msg);
		if (recv) {
			int err = errno;
			printw(
			    "MessagingListener %d: не удалось отправить "
			    "сообщение",
			    err);
			refresh();
		}
...
void* BroadcastListener(void* args) {
	...
	while (1) {
		int recv =
		    GetMsgFromServer(client->mqd_personal, &(client->list_msgs),
				     &(client->list_clients));
		if (recv) {
			int err = errno;
			printw(
			    "BroadcastListener %d: не удалось отправить "
			    "сообщение",
			    err);
			refresh();
		}
