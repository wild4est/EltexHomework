...
int InitQueue(mqd_t* mqd, const char* path, long maxmsg, long msgsize,
	...
	*mqd = mq_open(path, oflag | O_CREAT, 0666, &attr);
	if (*mqd == (mqd_t)-1) {
		//	printf("[X] InitQueue: Не получилось инициализировать
		// очередь\n");
	}
}
...
struct Client* InitClient(char* name) {
	...
	// InitQueue(&(client->mqd_messaging), QUEUE_MESSAGING, MSG_COUNT,
	// SIZE_BUFF_MSG, O_RDONLY);
	InitQueue(&(client->mqd_personal), name_for_queue, MSG_COUNT,
		  SIZE_BUFF_MSG, O_RDONLY);
	// InitQueue(&(client->mqd_exit), QUEUE_EXIT, MSG_COUNT, SIZE_BUFF_MSG,
	// O_RDONLY);
...
int GetDataFromServer(struct Client* client) {
	...
	recv =
	    mq_receive(client->mqd_personal, get_name, sizeof(get_name), NULL);
	if (recv == -1) {
		// printf("[X] GetDataFromServer: Не удалось принять сообщение
		// 1\n");
		return 1;
	}
	while (strcmp(get_name, ANSWER_END) != 0) {
		...
		ssize_t recv = mq_receive(client->mqd_personal, get_name,
					  sizeof(get_name), NULL);
		if (recv == -1) {
			// printf("[X] GetDataFromServer: Не удалось принять
			// сообщение 2\n");
			return 1;
		}
	}
	...
	while (strcmp(get_msg, ANSWER_END) != 0) {
		...
		ssize_t recv = mq_receive(client->mqd_personal, get_msg,
					  sizeof(get_msg), NULL);
		if (recv == -1) {
			// printf("[X] GetDataFromServer: Не удалось принять
			// сообщение 4\n");
...
int SendMsgToServer(mqd_t mqd_messaging, char* name, char* text_msg) {
	...
	if (recv == -1) {
		// printf("[X] MessagingListener: Не удалось отправить
		// сообщение\n");
		return 1;
	}
...
int GetMsgFromServer(mqd_t mqd_personal, struct List** list_msgs,
	...
	if (recv == -1) {
		// int err = errno;
		//	printf("[X] BroadcastListener: Не удалось принять
		// сообщение\n"); 	printf("Error = %d\n", err);
		return 1;
	}
