...
const char* BitapAlgorithm(const char* text, const char* pattern, int text_size, int pattern_size){
	...
	/* Для каждой буквы, которая присутсвует в паттерне, в маске при помощи нулей
	 * отображается её положение в паттерне */
	...
	for(int i=0; i<text_size; i++){
		// Наложение маски текущей буквы текста на битовый массив
		R |= alphabet_mask[*text];
		R <<= 1;

		/* Если в битовом массиве бит, который находится там же, где и заканчивается паттерн, является нулём,
		 * то это означает, что алгорим нашёл конец подстроки в тексте  */
