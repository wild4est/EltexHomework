 ...
 * \param struct Packet_UDP* packet_udp - указатель на структуру пакета, который будет заполняться
 * \param uint16_t port_source - потр отправителя
 * \param uint16_t port_destination - порт получателя
 * \param char* msg - сообщение
 */
void FillPacketUDP(struct Packet_UDP* packet_udp, uint16_t port_source, uint16_t port_destination, char* msg);

/*!
 * \brief Функция, отвечающая за заполнение UDP пакета на сетевом уровне
 * \parma struct Packet_IPV4* packet_ipv4 - указатель на структуру заголовка, который будет заполняться
 ...
 */
void FillPacketIPV4(struct Packet_IPV4* packet_ipv4, uint32_t ip_addr_source, uint32_t ip_addr_destination, struct Packet_UDP* packet_udp);
 ...
 * \param struct Packet_IPV4* packet_ipv4 - указатель на струтуру пакета ipv4, которй будет проверяться
 * \param uint32_t source_address - адресс отправителя
 * \param uint16_t source_port - порт отправителя
 * \return Число, обозначающее успех проверки. 1 - если адрес и порт получетеля (packet_ipv4) совпадает
 * с адресом и портом отправителя (source_address, source_port). 0 - адреса и порты не совпадают.
 */
