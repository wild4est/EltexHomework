...
void PrintLogo() {
	...
	printf(
	    "\e[1;34m\\\\\\\\\\\\____\\\\____\\\\_\\\\_\\\\____\\server\e["
	    "0m\n");
...
void FreeServer(struct Server* server) {
	...
	while (cur_node != NULL) {
		SendCode(&(cur_node->client), server->fd, server->addr.sin_port,
			 server->ip_addr, EXIT_CODE);
		printf("[!] port = %d | addr = %X\n",
		       ntohs(cur_node->client.addr.sin_port),
		       cur_node->client.addr.sin_addr.s_addr);
...
int GetMsg(struct Server* server, char* msg, struct sockaddr_in* addr) {
	while (1) {
		char buff[SIZE_BUFF];
		int recv_recvfrom =
		    recvfrom(server->fd, buff, sizeof(buff), 0, NULL, NULL);
		if (recv_recvfrom < 0) {
			int err = errno;
		...
		if (PacketIPV4Check(packet_ipv4, server->ip_addr,
			...
			printf(
			    "От пользователя[ addr = %X | port = %d] пришло "
			    "сообщение:\n",
...
int SendCode(struct Client* client, int fd_server, uint16_t server_port,
	     uint32_t server_ip_addr, char* code) {
	...
	FillPacketIPV4(&packet_ipv4, server_ip_addr,
		       client->addr.sin_addr.s_addr, &packet_udp);

	ssize_t recv_sendto =
	    sendto(fd_server, (char*)&packet_ipv4, sizeof(packet_ipv4), 0,
		   (struct sockaddr*)&(client->addr), len_client_addr);
	if (recv_sendto == -1) {
		int err = errno;
...
int SendMsg(struct Client* client, int fd_server, uint16_t server_port,
	    uint32_t server_ip_addr, char* buff) {
	...
	struct Packet_UDP packet_udp;
	FillPacketUDP(&packet_udp, server_port, client->addr.sin_port,
		      ans_buff);
	struct Packet_IPV4 packet_ipv4;
	FillPacketIPV4(&packet_ipv4, server_ip_addr,
		       client->addr.sin_addr.s_addr, &packet_udp);
...
void StartListeningMsg(struct Server* server) {
	...
	while (1) {
		...
		if (IsItExitMsg(msg)) {
			...
			printf(
			    "Пользователь[ addr = %X | port = %d ] покинул "
			    "сервер\n",
		...
		struct Client* search_client =
		    SearchClient(server->list_clients, &client_addr);
		if (search_client == NULL) {
			...
			SendMsg(&new_client, server->fd, server->addr.sin_port,
				server->ip_addr, msg);
		} else {
			printf(
			    "Это оказался старый пользователь (msg_count = "
			    "%d)\n",
			    search_client->msg_count);
			search_client->msg_count++;
			SendMsg(search_client, server->fd,
				server->addr.sin_port, server->ip_addr, msg);
		}
