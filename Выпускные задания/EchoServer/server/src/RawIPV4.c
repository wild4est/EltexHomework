...
void FillPacketIPV4(struct Packet_IPV4* packet_ipv4, uint32_t ip_addr_source,
	...
	// inet_pton(AF_INET, IP_ADDR_SRC, (struct
	// in_addr*)&(packet_ipv4->source_address)); inet_pton(AF_INET,
	// ip_addr_server, (struct
	// in_addr*)&(packet_ipv4->destination_address));
...
int PacketIPV4Check(struct Packet_IPV4* packet_ipv4, uint32_t source_address,
	...
	int port_check =
	    packet_ipv4->packet_udp.destination_port == source_port;
