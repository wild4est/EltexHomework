...
int SendMsg(struct Client* client, char* msg) {
	...
	FillPacketIPV4(&packet_ipv4, client->ip_addr,
		       client->addr.sin_addr.s_addr, &packet_udp);

	ssize_t recv_send =
	    sendto(client->fd, (char*)&packet_ipv4, sizeof(packet_ipv4), 0,
		   (struct sockaddr*)&(client->addr), len_addr);
	if (recv_send == -1) {
...
int GetMsg(struct Client* client) {
	while (1) {
		char buff[SIZE_BUFF];
		ssize_t recv_get =
		    recvfrom(client->fd, buff, sizeof(buff), 0, NULL, NULL);
		if (recv_get == -1) {
			return -1;
		}
		...
		if (PacketIPV4Check(packet_ipv4, client->ip_addr,
				    client->port)) {
			if (IsItExitMsg(packet_ipv4->packet_udp.data)) {
				return 1;
			}
			if (client->list_msg->length >=
			    client->list_msg->max_length) {
