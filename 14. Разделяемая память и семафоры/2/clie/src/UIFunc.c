...
void* MessagingListener(void* args){
	...
	while(1){
		...
		int recv = SendMsgToServer(&(client->shared_mem_messaging), client->name, msg);
		if (recv) {
			int err = errno;
...
void* BroadcastListener(void* args){
	struct Client* client = (struct Client*)args;
	while(1){
		int recv = GetMsgFromServer(&(client->shared_mem_personal), &(client->list_msgs), &(client->list_clients));
		if (recv) {
			int err = errno;
