...
struct Client* InitClient(char* name) {
	...
	InitShm(&(client->shared_mem_messaging.shm),
		&(client->shared_mem_messaging.ptr), SHM_MESSAGING, O_RDWR,
		SIZE_BUFF_MSG);
	InitShm(&(client->shared_mem_personal.shm),
		&(client->shared_mem_personal.ptr), name_for_shm,
		O_CREAT | O_RDWR, SIZE_BUFF_MSG);
	InitShm(&(client->shared_mem_exit.shm), &(client->shared_mem_exit.ptr),
		SHM_EXIT, O_RDWR, SIZE_BUFF_MSG);
	...
	client->shared_mem_personal.sem_read =
	    sem_open(name_for_sem, O_CREAT, 0666, 0);
	...
	client->shared_mem_personal.sem_write =
	    sem_open(name_for_sem, O_CREAT, 0666, 1);
...
int SendMsgToServer(struct SharedMemory* shared_mem, char* name,
		    char* text_msg) {
