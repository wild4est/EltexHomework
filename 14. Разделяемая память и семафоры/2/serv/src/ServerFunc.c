...
struct Server* InitServer(){
	...
	serv->shared_mem_new_client.sem_read = sem_open(SEM_NEW_CLIENT_RD, O_CREAT, 0666, 0);
	serv->shared_mem_messaging.sem_read = sem_open(SEM_MESSAGING_RD, O_CREAT, 0666, 0);
	serv->shared_mem_exit.sem_read = sem_open(SEM_EXIT_RD, O_CREAT, 0666, 0);
	
	serv->shared_mem_new_client.sem_write = sem_open(SEM_NEW_CLIENT_WR, O_CREAT, 0666, 1);
	serv->shared_mem_messaging.sem_write = sem_open(SEM_MESSAGING_WR, O_CREAT, 0666, 1);
	serv->shared_mem_exit.sem_write = sem_open(SEM_EXIT_WR, O_CREAT, 0666, 1);

	InitShm(&(serv->shared_mem_new_client.shm), &(serv->shared_mem_new_client.ptr), SHM_NEW_CLIENT, SIZE_BUFF_NAME+1);
	InitShm(&(serv->shared_mem_messaging.shm), &(serv->shared_mem_messaging.ptr), SHM_MESSAGING, SIZE_BUFF_NAME+1);
	InitShm(&(serv->shared_mem_exit.shm), &(serv->shared_mem_exit.ptr), SHM_EXIT, SIZE_BUFF_NAME+1);
...
int OpenQueueToClient(char* name, struct SharedMemory* shared_mem){
	...
	shared_mem->ptr = mmap(0, SIZE_BUFF_MSG, PROT_WRITE, MAP_SHARED, shared_mem->shm, 0);
...
void SendDataToClient(struct SharedMemory shared_mem, struct ListClient* list_client, struct ListMsg* list_msg){
	int recv;
...
void* NewClientListener(void* args){
		...
		SendMsgToAllClients(serv->list_clients, msg);
	       	...
	       	SendDataToClient(shared_mem_client, serv->list_clients, serv->list_msgs);
